...
def home(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if item.status == Item.Status.PROCESSED:
            messages.success(request, 'Nota salva no Segundo Cérebro.')
        else:
...
def library(request):
    items = Item.objects.all().prefetch_related('tags', 'topics', 'projects')
    ...
    if q:
        items = items.filter(
            ...
            Q(source_author__icontains=q) | Q(tags__name__icontains=q) |
            Q(topics__name__icontains=q) | Q(projects__name__icontains=q)
        ).distinct()
...
def item_detail(request, pk):
    item = get_object_or_404(
        Item.objects.prefetch_related('tags', 'topics', 'projects', 'chunks'), pk=pk
    )
    ...
    return render(request, 'knowledge/item_detail.html', {'item': item, 'relations': relations})
...
def toggle_favorite(request, pk):
    ...
    return HttpResponseRedirect(request.META.get('HTTP_REFERER') or item.get_absolute_url())


def connections(request):
    relations = Relation.objects.filter(status=Relation.Status.CONFIRMED).select_related('source', 'target')[:250]
